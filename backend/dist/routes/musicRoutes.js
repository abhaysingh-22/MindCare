...
class MusicRoutes {
    constructor(musicService, moodService, streamingService) {
    ...
    async logMood(req, res) {
        try {
            const { userId, moodScore, moodCategory, activity, notes } = req.body;
            ...
            const logId = await this.moodService.logMood(userId, moodScore, moodCategory, activity, notes);
            res.json({
                success: true,
                logId,
    ...
    async getRecommendations(req, res) {
        try {
            const { user_id: userId, mood_category: moodCategory, energy_level: energyLevel, genre_preference: genrePreference } = req.body;
            if (!userId) {
                return res.status(400).json({
                    error: 'User ID is required'
                });
            ...
            let recommendations;
            if (moodCategory) {
                recommendations = await this.musicService.getRecommendations(userId, moodCategory, energyLevel);
            }
            else {
     ...
     */
    async getCuratedMusic(req, res) {
        try {
            const mood = req.params.mood;
            const genre = req.query.genre;
            const energyLevel = req.query.energyLevel ? parseInt(req.query.energyLevel) : undefined;
            const limit = req.query.limit ? parseInt(req.query.limit) : 20;
            const tracks = await this.streamingService.getCuratedTracks(mood, genre, energyLevel, limit);
            res.json({
                tracks,
                mood,
                genre,
    ...
    async playMusic(req, res) {
        try {
            ...
            const tracks = await this.streamingService.getCuratedTracks(moodResult.mood_category, undefined, // Let service determine genre based on preferences
    ...
    async searchSpotify(req, res) {
        try {
            ...
            const tracks = await this.streamingService.searchSpotifyTracks(mood, genre, energyLevel, limit);
            res.json({
                tracks
            });
        }
    ...
    async searchYouTube(req, res) {
        try {
            ...
            const tracks = await this.streamingService.searchYouTubeTracks(mood, genre, limit);
            res.json({
                tracks
            });
        }
    ...
    async createPlaylist(req, res) {
        try {
            const { userAccessToken, playlistName, trackIds } = req.body;
            ...
            const playlistId = await this.streamingService.createSpotifyPlaylist(userAccessToken, playlistName, trackIds);
            if (playlistId) {
                res.json({
                    success: true,
                    playlistId,
     ...
     */
    async startMusicSession(req, res) {
        try {
            const { userId, trackId, moodBefore } = req.body;
    ...
    async submitFeedback(req, res) {
        try {
            const { sessionId, moodAfter, feedbackRating } = req.body;
