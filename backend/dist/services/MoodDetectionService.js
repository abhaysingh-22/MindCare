...
exports.MoodDetectionService = void 0;
class MoodDetectionService {
    constructor(database, musicPreferencesService) {
        this.db = database;
    ...
    async detectMood(userId) {
        ...
        const recommendations = await this.musicService.getRecommendations(userId, currentMood.mood_category, currentMood.energy_level);
        return {
     ...
     */
    async logMood(userId, moodScore, moodCategory, activity, notes) {
        const database = this.db.getDatabase();
        return new Promise((resolve, reject) => {
            database.run(`INSERT INTO mood_logs (user_id, mood_score, mood_category, activity, notes, detected_automatically)
         VALUES (?, ?, ?, ?, ?, ?)`, [userId, moodScore, moodCategory, activity || null, notes || null, false], function (err) {
                if (err) {
                    reject(err);
                    return;
                }
     ...
     */
    async automaticMoodDetection(userId, behaviorData) {
        ...
        const recommendations = await this.musicService.getRecommendations(userId, detectedMood.mood_category, detectedMood.energy_level);
        return {
     ...
     */
    async getRecentMoodLogs(userId, days) {
        const database = this.db.getDatabase();
        return new Promise((resolve, reject) => {
            database.all(`SELECT * FROM mood_logs 
         WHERE user_id = ? AND timestamp >= datetime('now', '-${days} days')
         ORDER BY timestamp DESC`, [userId], (err, rows) => {
                if (err) {
                    reject(err);
                    return;
                }
     ...
     */
    async analyzeMoodPattern(moodLogs) {
     ...
     */
    analyzeBehaviorForMood(behaviorData) {
        ...
        const { appUsageTime = 0, lastActivityTime = Date.now(), musicListeningHistory = [], timeOfDay = new Date().getHours() } = behaviorData;
     ...
     */
    async logAutomaticMood(userId, detectedMood) {
        const database = this.db.getDatabase();
        return new Promise((resolve, reject) => {
            database.run(`INSERT INTO mood_logs (user_id, mood_score, mood_category, detected_automatically, notes)
         VALUES (?, ?, ?, ?, ?)`, [
                userId,
     ...
     */
    calculateConfidence(moodLogs, predictedMood) {
        if (moodLogs.length === 0)
            return 0.3;
     ...
     * Determine energy level based on mood category and score
     */
    determineEnergyLevel(moodCategory, moodScore) {
    ...
    async getMoodStatistics(userId, days = 30) {
        ...
        if (secondHalfAvg - firstHalfAvg > 0.5)
            ...
            trend = 'declining';
        const dominantMood = Object.keys(moodDistribution).reduce((a, b) => moodDistribution[a] > moodDistribution[b] ? a : b);
        return {
...
exports.MoodDetectionService = MoodDetectionService;
//# sourceMappingURL=MoodDetectionService.js.map