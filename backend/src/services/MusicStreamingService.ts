...
export class MusicStreamingService {
  ...
  async searchSpotifyTracks(
    ...
    try {
      ...
      if (trackIds.length > 0) {
        ...
        featuresResponse.data.audio_features.forEach((features: any, index: number) => {
   ...
   */
  private buildSpotifySearchQuery(mood: MoodCategory, genre?: string, energyLevel?: number): string {
   ...
   */
  private estimateEnergyFromSpotifyFeatures(track: any): number {
   ...
   */
  private estimateValenceFromMood(mood: MoodCategory): number {
    const valenceMap: Record<MoodCategory, number> = {
      happy: 0.8,
      sad: 0.2,
      calm: 0.5,
      energetic: 0.7,
   ...
   */
  private buildYouTubeSearchQuery(mood: MoodCategory, genre?: string): string {
    const moodQueries: Record<MoodCategory, string> = {
   ...
   */
  private estimateEnergyFromMood(mood: MoodCategory): number {
    const energyMap: Record<MoodCategory, number> = {
      happy: 7,
      sad: 3,
      calm: 4,
      energetic: 9,
   ...
   */
  async storeTracksInDatabase(tracks: Partial<MusicTrack>[]): Promise<number[]> {
  ...
  async getCuratedTracks(
    ...
    const localTracks = await new Promise<MusicTrack[]>((resolve, reject) => {
